...
export function ViewByFirstname() {
   ...
   const FirstNameHandler =async(event)=> {

    if (event.target.value[0] >= 'A' && event.target.value[0] <= 'Z') {
      document.getElementById("viewfirstNamelabel").className = 'hidden';
  }
  else {
      document.getElementById("viewfirstNamelabel").className = '';
  }
     ...
     await axios.get(`http://localhost:8080/cc/firstname/${FirstName}`) 
      .then((result) => {
...
<div1>
        <br></br><br></br><br></br>
        <form className='form' onSubmit={()=>getdata({FirstName})}>
        <h2 Style="padding-left:180px;color:blue;margin-top:2px;font-style:italic;font-family:TheSpeakeasy"><u>Search By FirstName</u>  </h2><br></br>
        <table className='table'>

        <label id="viewfirstNamelabel" class="hidden">First letter should be Capital</label>
                        <tr>
                            <td><label>FirstName: </label></td>
                            <td><input type="text" Placeholder="Enter the FirstName" value={FirstName} onChange={FirstNameHandler} required pattern="[A-Z]{1}[a-z]{1,}"></input>
                            </td>
                        </tr>
            </table> 
            <br></br><br></br><br></br>
      ...
      <div2>
    <br></br><br></br><br></br>
   <h1 Style="color:green;padding-left:900px"><u>CustomerInfoByFirstName</u></h1><br></br><br></br>
...
export default ViewByFirstname