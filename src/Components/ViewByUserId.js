...
export function ViewByUserId() {
   ...
   const UserIdHandler =async(event)=> {
     ...
     if (event.target.value > 99999) {
       document.getElementById("viewUserIdlabel").className = '';
     }
     else {
       document.getElementById("viewUserIdlabel").className = 'hidden';
     }
     ...
     await axios.get(`http://localhost:8080/cc/userid/${UserId}`) 
      .then((result) => {
...
<div1>
        <br></br><br></br><br></br>
        <form className='form' onSubmit={()=>getdata({UserId})}>
        <h2 Style="padding-left:180px;color:blue;margin-top:2px;font-style:italic;font-family:TheSpeakeasy"><u>Search By UserId</u>  </h2><br></br>
        <table className='table'>

        <label id="viewUserIdlabel" class="hidden">Should be 5 digits</label>
                        <tr>
                            <td><label>UserId: </label></td>
                            <td><input type="text" Placeholder="Enter the UserId" value={UserId} onChange={UserIdHandler} required pattern="[1-9]{1}[0-9]{4,4}"></input>
                            </td>
                        </tr>
            </table> 
            <br></br><br></br><br></br>
      ...
      <div2>
    <br></br><br></br><br></br>
   <h1 Style="color:green;padding-left:900px"><u>CustomerInfoByUserId</u></h1><br></br><br></br>
